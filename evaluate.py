...
parser.add_argument('--name_result_file', type=str , default= "result.txt" , help = 'neme of file')
...
parser.add_argument('--train', type=int , default= 1 , help = 'if 1 train and test if zero just test')
parser.add_argument('--baselines', type=list_of_strings , default= ['densenet121', 'resnet34','squeezenet1.1'] , help ='list of baselines' )
...
def evaluate(net, preprocess , model_name , adj):
    ...
    if os.path.isfile(os.path.join(args.output_path , adj ,model_name, args.name_result_file)) == False:
        f = open(os.path.join(args.output_path ,adj, model_name , args.name_result_file), "x")
    f = open(os.path.join(args.output_path , adj, model_name , args.name_result_file), "w")
...
if args.adjective_flag == 1:
    ...
    for i, model in enumerate(baselines):
        ...
        for adj in args.adjective_list:
            precision_avg , sensitivity_avg , specifity_avg , test_accuracy = evaluate(net, preprocess , model_name, adj)
            F1_avg = (precision_avg * sensitivity_avg) / (precision_avg + sensitivity_avg)
    ...
    csv_file_name = os.path.join( args.output_path , args.name_result_file + '_table.csv')
    ...
    for i, model in enumerate(baselines):
        ...
        precision_avg , sensitivity_avg , specifity_avg , test_accuracy = evaluate(net, preprocess , model_name , "")
        F1_avg = (precision_avg * sensitivity_avg) / (precision_avg + sensitivity_avg)
        table_data.append([model_name , '' , test_accuracy , sensitivity_avg , specifity_avg , precision_avg , F1_avg ])

    # Specify the CSV file name
    csv_file_name = os.path.join( args.output_path , 'result_table.csv')
